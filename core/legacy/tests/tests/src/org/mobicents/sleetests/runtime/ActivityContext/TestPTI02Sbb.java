...
/**
 * Tests if detaching and then attaching an sbb entity to the AC where the event was received causes the sbbEntity to receive it again. 
 * @author Eduardo Martins
 *
 */
public abstract class TestPTI02Sbb extends BaseTCKSbb {
	...
	public void onTCKResourceEventX1(TCKResourceEventX event,
		...
		System.out.println("Event X1 received!");
		try {
			if (getValue() == false) {
				setValue(true);
				...
				map.put("Message", "Sbb Entity received the event X1 twice.");
	...
	public void onTCKResourceEventX2(TCKResourceEventX event,
			ActivityContextInterface aci) {
		System.out.println("Event X2 received!");
		try {			
				HashMap map = new HashMap();
				map.put("Result", new Boolean(false));
				map.put("Message", "Sbb Entity didn't receive the event X1 twice.");
				TCKSbbUtils.getResourceInterface().sendSbbMessage(map);							
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
		}		
