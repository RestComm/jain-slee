...
public class SbbObjectPoolManagement implements SbbObjectPoolManagementMBean {
	...
	private final static Logger logger = Logger.getLogger(SbbObjectPoolManagement.class);
	 ...
	 * transaction manager is used then, and if the tx rollbacks, the pool will be
	 * removed.
	 * @param sbbDescriptor
	 * @param sleeTransactionManager
	 */
	public void createObjectPool(final MobicentsSbbDescriptor sbbDescriptor, final SleeTransactionManager sleeTransactionManager) {
		...
		if (sleeTransactionManager != null) {
			...
			TransactionalAction action = new TransactionalAction() {
				public void execute() {
					if (logger.isDebugEnabled()) {
						logger.debug("Due to tx rollback, removing pool for sbb "+ sbbDescriptor.getID());
					}
					try {
						removeObjectPool(sbbDescriptor.getID());
					} catch (Exception e) {
						logger.error("Failed to remove SBB "
								+ sbbDescriptor.getID() + " object pool",
								e);
					}
				}
			};
	...
	private void createObjectPool(final MobicentsSbbDescriptor sbbDescriptor) {
		// create the pool for the given SbbID
		GenericObjectPoolFactory poolFactory = new GenericObjectPoolFactory(new SbbObjectPoolFactory(sbbDescriptor),config);
		final ObjectPool objectPool = poolFactory.createPool();
		final ObjectPool oldObjectPool = pools.put(sbbDescriptor.getID(),objectPool);
	 ...
	 * @throws Exception
	 */
	public void removeObjectPool(final MobicentsSbbDescriptor sbbDescriptor, final SleeTransactionManager sleeTransactionManager) throws Exception {
		...
		if (sleeTransactionManager != null) {
			...
			TransactionalAction action = new TransactionalAction() {				
				public void execute() {
					if (logger.isDebugEnabled()) {
						logger.debug("Due to tx rollback, restoring pool for sbb "+ sbbDescriptor.getID());
					}
					createObjectPool(sbbDescriptor);
				}
			};
	...
	public void reconfig() {
		for (Object key : pools.keySet().toArray()) {
			final MobicentsSbbDescriptor sbbDescriptor = (MobicentsSbbDescriptor) sleeContainer
					.getComponentDescriptor((ComponentID)key);
			createObjectPool(sbbDescriptor);
		}
	}
}
