...
public class Service implements Serializable {
    ...
    private static HashMap getUsageParameters(ServiceID serviceId) {
        ...
        if (hashMap == null) {
        	hashMap = new HashMap();
            HashMap anotherHashMap = usageParameters.putIfAbsent(serviceId, hashMap);
    ...
    public void setState(ServiceState serviceState) {
        if (logger.isDebugEnabled()) {
            try {
            	ServiceState oldServiceState = (ServiceState) serviceAttributes.get(SERVICE_STATE);
				logger.debug("ServiceComponent.setState(): State service ID =  "
				        + this.serviceID + " current State = " + oldServiceState
				        + " new State = " + serviceState + " TX ID: " + SleeContainer.getTransactionManager().getTransaction());
    ...
    private Service(ServiceDescriptorImpl serviceDescriptor)
        ...
        try {
            ...
            childObj = new CacheableMap(tcache + "-" + getCacheNodeName() + "#" + CHILD_OBJ);

            serviceAttributes = new CacheableMap(tcache + "-" + getCacheNodeName() + "#" + SERVICE_ATTRS);
    ...
    public String getCacheNodeName() {
        if (this.cacheNodeName != null)
            ...
            this.cacheNodeName = "services:" + serviceID.getComponentKey().getName()
    ...
    /**
     * Get the service descriptor. Note that the ServiceComponent is the stuff
     * that is installed (it is static and defined by the XML Descriptor for the
     * Service).
     *  
     */
    private ServiceComponent getServiceComponent() {
        return sleeContainer.getServiceComponent(this.serviceID);
    }
    ...
    public SbbEntity addChild(String convergenceName) throws CreateException {
        ...
        SbbEntity sbbEntity = SbbEntityFactory.createRootSbbEntity(
                        (SbbID) this.getServiceComponent()
    ...
    public void activate() throws SystemException {
    	...
    	ActivityContext ac = sleeContainer.getActivityContextFactory().getActivityContext(serviceActivityImpl);
    	...
    	// create event
        int eventTypeID = sleeContainer.getEventLookupFacility().getEventID(
                "javax.slee.serviceactivity.ServiceStartedEvent", "javax.slee",
                "1.0");
    ...
    public static Service getService(ServiceDescriptorImpl serviceDescriptorImpl) {
    	...
    	// check local tx data
    	Service service = (Service) txMgr.getTxLocalData(serviceDescriptorImpl.getID());
