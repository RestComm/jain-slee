...
class SbbComponentDeployer extends AbstractComponentDeployer {
    ...
    protected List parseComponentDescriptors()
        ...
        List sbbNodes = XMLUtils.getAllChildElements(sbbJarNode, XMLConstants.SBB_ND);
        if(sbbNodes.size() == 0){
        ...
        */
        ...
        if (ddMobicentsXmlEntry != null)
        {
	        ...
	        try {
	            ...
	            throw new DeploymentException("Failed to extract the Mobicents SBB deployment"
	                +" descriptor from " + jar.getName());
	        }
	        ...
	        sbbMobicentsNodes =
	            XMLUtils.getAllChildElements(sbbMobicentsJarNode, XMLConstants.MC_SBB_ND);
        ...
        for (int i = sbbNodes.size() - 1; i >= 0; i--) {
            ...
            MobicentsSbbDescriptorInternalImpl descriptor = 
                (MobicentsSbbDescriptorInternalImpl) parser.parseSbbComponent(sbbNode, new MobicentsSbbDescriptorInternalImpl());
