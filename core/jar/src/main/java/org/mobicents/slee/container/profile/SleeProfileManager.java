...
public class SleeProfileManager extends ServiceMBeanSupport implements SleeProfileManagerMBean {
    ...
    class ProfileCacheManager  {
        ...
        public synchronized Set getProfileTableNames() {
        	if (profileTableNames == null) {
            	this.profileTableNames = new CacheableSet(tcache + "-" + PROFILE_TABLE_NAMES);
        	};
    ...
    /**
     * Microkernel callback, which initializes service state.
     */
    ...
    public static synchronized SleeProfileManager getInstance() {
        if (sleeProfileManager == null) throw new SLEEException("Access attempted before initialization of " + SleeProfileManager.class.getName());
     ...
     */
    protected synchronized ObjectName instantiateProfile(String cmpInterfaceName,
            String profileTableName, String profileName,
            boolean loadDataFromBackendStorage) throws Exception {
        ...
        if (loadDataFromBackendStorage) {
            profileManagementInterceptor
                    .loadStateFromBackendStorage(cmpInterfaceName);
        } else {
            if (isDefaultProfile) {
                ...
                profileManagementInterceptor.copyStateFromDefaultProfile(
                        cmpInterfaceName, defaultProfileKey);
            }
        }
        ...
        Class concreteProfileMBeanClass = Thread.currentThread()
                .getContextClassLoader().loadClass(mbeanConcreteName);
        ...
        mbeanServer.registerMBean(profileMBean, objectName);
        return objectName;
    }
    ...
    public ObjectName addProfileToProfileTable(String profileTableName,
        ...
        try {
            ...
            Set registredMBeans = (Set) transactionManager.getObject(
                    tcache, profileTableKey, "registredMBeans");
            if (registredMBeans != null) {
            	registredMBeans.add(objectName);
            }
            else {
            	if(logger.isDebugEnabled()) {
            		logger.warn("registred mbeans set of profile table not found in cache");
            	}
            }
    ...
    public ProfileManagement instantiateLastCommittedProfile(
        ...
        if(logger.isDebugEnabled()) {
        logger.debug("instantiating Last Committed Profile to fire an event"
                + "profileTableKey" + profileTableKey + ",profileKey="
                + profileKey);
        }
        
        String cmpInterfaceName = (String) sleeProfileManager
        ...
        Class concreteProfileClass = Thread.currentThread()
                .getContextClassLoader().loadClass(cmpConcreteName);
        Constructor constructor = concreteProfileClass
                .getConstructor(new Class[] {
                        ProfileManagementInterceptor.class,
                        SleeProfileManager.class, String.class });
     ...
     *             if a low level exception occurs
     */
    public synchronized void removeProfile(String profileTableName, String profileName)
            throws SystemException {
        ...
        try {
            ...
            String profileTableKey = this.generateProfileTableKey(profileTableName);
            String profileKey = this.generateProfileKey(profileTableName, profileName);
            ...
            Set profileTableRegistredMbeans = (Set)transactionManager.getObject(tcache,profileTableKey,"registredMBeans");
...
*/
    ...
    public Collection getProfilesByIndexedAttribute(String profileTableName,
        ...
        try {
            Class profileTransientClass = Class
                    .forName(ConcreteClassGeneratorUtils.PROFILE_TRANSIENT_CLASS_NAME_PREFIX
                            + cmpInterfaceName
                            + ConcreteClassGeneratorUtils.PROFILE_TRANSIENT_CLASS_NAME_SUFFIX);
            profileTransientClass
                    .getField(attributeName);
    ...
    public void setProfileAttributeValue(String profileKey,
        ...
        if (profileKey
                .startsWith(getRootFqn() + SleeProfileManager.PROFILEID_LOOKUP_PREFIX)) {
            verifyProfileIndex(profileKey, profileAttributeName, profileAttributeValue, transactionManager);
        } // index check
     ...
     */
    private void verifyProfileIndex(String profileKey, String profileAttributeName, 
            Object profileAttributeValue, SleeTransactionManager transactionManager)
        throws SystemException, ProfileVerificationException {
        String intermediateKey = profileKey.substring((getRootFqn() + PROFILEID_LOOKUP_PREFIX)
                .length());
        String profileTableName = intermediateKey.substring(0,
                intermediateKey.indexOf("/"));
        if(logger.isDebugEnabled()) {
        	logger.debug("setProfileAttributeValue: profileTableName = " + profileTableName);
        }
        ...
        if  ( logger.isDebugEnabled()) {
            logger.debug("setProfileAttributeValue: indexedAttributes : " + indexedAttributes);
        }
        if (indexedAttributes != null) {
                    ...
                    while (keys.hasNext()) {
                        ...
                        if (profileAttributeValue != null) {
                            ...
                            if (profileAttributeValue.getClass().isArray()) {
                            	...
                            	if(logger.isDebugEnabled()) {
                            		logger.debug(indexKey + " is not an array!!!!");
                            	}
    ...
    public Map getProfileIndexesSpec(String profileTableName) throws SystemException {
        Map indexes = (Map)getTransactionManager().getObject(
                tcache, generateProfileTableKey(profileTableName), PROFILE_INDEXES_SPEC_SUFFIX);
        return indexes;
    }
    ...
    public synchronized void unregisterProfileMBean(String profileKey) throws Exception {
        ...
        ObjectName objectName = null;
        if (profileKey
                .startsWith(SleeProfileManager.PROFILEID_LOOKUP_PREFIX)) {
    ...
    public synchronized ProfileTableActivityImpl createProfileTableActivity(
        ...
        if ((profileTableActivity = (ProfileTableActivityImpl) this.profileTableActivities
	...
	public Object loadObjectFromCache(String fqn, String key) throws SystemException {
        return getTransactionManager().getObject(tcache, fqn, key);
	}
	...
	public Object loadNodeFromCache(String fqn) throws SystemException {
        return getTransactionManager().getNode(tcache, fqn);
	}
