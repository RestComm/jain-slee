...
public class SleeProfileManager {
	...
	public void addProfileTable(String profileTableName,
		...
		try {
			...
			ProfileSpecificationDescriptorImpl profileSpecificationDescriptorImpl = (ProfileSpecificationDescriptorImpl) profileSpecificationDescriptor;
			profileTableCacheData.setProfileTableData(profileSpecificationDescriptorImpl.getProfileSpecificationID(), profileSpecificationDescriptorImpl.getCMPInterfaceName(), profileSpecificationDescriptorImpl.getSingleProfile(), profileSpecificationDescriptorImpl.getProfileIndexes());
			if (logger.isDebugEnabled()) {
			...
			for (Object obj : indexes.keySet()) {
				String attributeName = (String) obj;
				Class profileTransientClass = Thread.currentThread().getContextClassLoader().loadClass(ConcreteClassGeneratorUtils.PROFILE_TRANSIENT_CLASS_NAME_PREFIX	+ profileSpecificationDescriptor.getCMPInterfaceName() + ConcreteClassGeneratorUtils.PROFILE_TRANSIENT_CLASS_NAME_SUFFIX);
				...
				Boolean isUnique = (Boolean) indexes.get(attributeName);
			...
			instantiateProfile(profileSpecificationDescriptor.getCMPInterfaceName(), profileTableName, null, false,profileTableCacheData);
			if (logger.isDebugEnabled()) {
	...
	public void removeProfileTable(String profileTableName)
		...
		try {	
			...
			ObjectName defaultProfileObjectName = ProfileProvisioningMBeanImpl
			...
			if (sleeContainer.getMBeanServer().isRegistered(defaultProfileObjectName)) {
				logger.info("Unregistering following "
						+ "profile table MBean with object name " + defaultProfileObjectName);
	...
	public ObjectName addProfileToProfileTable(String profileTableName,
		...
		try {
			...
			objectName = instantiateProfile(profileTableCacheData.getCmpInterfaceName(), profileTableName,newProfileName, false,profileTableCacheData);
	 ...
	 */
	protected ObjectName instantiateProfile(
			String cmpInterfaceName, String profileTableName,
			String profileName, boolean loadDataFromBackendStorage, ProfileTableCacheData profileTableCacheData)
			throws Exception {
		...
		Constructor constructor = concreteProfileClass
				.getConstructor(new Class[] {
						...
						SleeProfileManager.class, String.class, String.class });
		...
		if (loadDataFromBackendStorage) {
			...
			if (isDefaultProfile) {
				...
				profileManagementInterceptor.copyStateFromDefaultProfile(
						cmpInterfaceName, profileSpecDUClassLoader);
			}
		}
	 ...
	 *             if a low level exception occurs
	 */
	public void removeProfile(String profileTableName,
		...
		try {
			...
			ProfileRemovedEventImpl profileRemovedEvent = new ProfileRemovedEventImpl(
					...
					profileManagement, profileTableActivityContextInterfaceFactory);
			...
			ActivityContext ac = sleeContainer.getActivityContextFactory().getActivityContext(ActivityContextHandlerFactory.createProfileTableActivityContextHandle(new ProfileTableActivityHandle(profileTableName)), false);
			DeferredEvent de = new DeferredEvent(profileRemovedEvent.getEventTypeID(),
					profileRemovedEvent, ac, profileAddress);
	 ...
	 */
	public ObjectName findProfileMBean(String profileTableName,
			String profileName) {
		if (logger.isDebugEnabled()) {
	 ...
	 *         SystemException
	 */
	public Set<String> getProfilesByIndexedAttribute(String profileTableName,
	...
	public Object getSbbCMPProfile(ProfileID profileID) throws SystemException {
		...
		try {
			...
			return constructor.newInstance(new Object[] {
					profileManagementInterceptor, this, profileID.getProfileTableName(), profileID.getProfileName() });
		} catch (Exception e) {
	...
	public void setProfileAttributeValue(String profileTableName, String profileName,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("setProfileAttributeValue ( profileTable = "
					+ profileTableName + " profileName = "+ profileName+ " profileAttributeName = "
		...
		// also the profile name must be for a non-default profile. Default profiles are not indexed, only persisted.
		if (profileName != null) {
			verifyProfileIndex(profileTableName, profileName, profileAttributeName,
					profileAttributeValue, transactionManager, profileTableCacheData);
		} 
	 ...
	 */
	private void verifyProfileIndex(String profileTable, String profileName,
			String profileAttributeName, Object profileAttributeValue,
			SleeTransactionManager transactionManager, ProfileTableCacheData profileTableCacheData) throws SystemException,
			ProfileVerificationException {
		
		if (logger.isDebugEnabled()) {
			logger.debug("verifyProfileIndex: profileTable = "
					+ profileTable+" , profileName = "+profileName+" , attributeName = "+profileAttributeName+" , attributeValue = "+profileAttributeValue);
		}
		
		if (profileTableCacheData.isIndexedAttribute(profileAttributeName)) {
			
			if (logger.isDebugEnabled()) {
				logger.debug("Found indexed attr profileTable = "
					+ profileTable+" , profileName = "+profileName+" , attributeName = "+profileAttributeName);
			}
			...
			//if it is so then an exception is thrown
				...
				if (indexMap.keySet().size() < 1) {
					if (logger.isDebugEnabled()) {
						logger.debug("indexed attr profileTable = "
							+ profileTable+" , profileName = "+profileName+" , attributeName = "+profileAttributeName
					...
					Iterator keys = new HashSet(indexMap.keySet())
					.iterator();
					while (keys.hasNext()) {
						String indexedProfileName = (String) keys.next();
						Object profileIndexedAttributeValue = indexMap
	...
	public Object getProfileAttributeValue(String profileTableName, String profileName,
	...
	public void unregisterProfileMBean(String profileTableName, String profileName)
			throws Exception {
