...
public class DeploymentCacheManager {
	...
	private Logger logger = Logger.getLogger(DeploymentCacheManager.class
			.getName());

	@Override
	public String toString() {
		return 	"Deployment Cache: " +
				"\n+-- URLS: " + urlToDeployableUnitIDMap.keySet() +		
				"\n+-- Deployable Units IDs: " + deployableUnitIDtoDescriptorMap.keySet() +
				"\n+-- Referring Components: " + referringComponents.keySet() +
				"\n+-- Component IDs: " + componentIDToDeployableUnitIDMap.keySet() +
				"\n+-- SBB Components: " + sbbComponents.keySet() +
				"\n+-- Profile Components: " + profileComponents.keySet() +
				"\n+-- Service Components: " + serviceComponents.keySet() +
				"\n+-- Service RA Entities: " + serviceResourceAdaptorEntities.keySet() +
				"\n+-- Active Service IDs: " + activeServiceIDs;
	}
	...
	public DeploymentCacheManager() {
		this.serviceComponents = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ SERVICE_COMPONENTS);
		this.serviceResourceAdaptorEntities = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ SERVICE_RESOURCE_ADAPTOR_ENTITIES);
		this.referringComponents = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ REFERRING_COMPONENTS);
		this.profileComponents = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ PROFILE_COMPONENTS);
		// Get the various MBeans that should be initialized by now.
		this.componentIDToDeployableUnitIDMap = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ COMPONENT_ID_TO_DEPLOYABLE_UNIT_ID_MAP);
		this.sbbComponents = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":" + SBB_COMPONENTS);
		this.urlToDeployableUnitIDMap = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ URL_TO_DEPLOYABLE_UNIT_ID_MAP);
		this.deployableUnitIDtoDescriptorMap = new CacheableMap(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ DU_ID_TO_DESCRIPTOR_MAP);
		this.activeServiceIDs = new CacheableSet(
				TransactionManagerImpl.DEPLOYMENT_CACHE + ":"
						+ ACTIVE_SERVICE_IDS);
	}
	...
	public Set newReferringDuSet() {
		Set refs = new CacheableSet(TransactionManagerImpl.DEPLOYMENT_CACHE
				+ ":" + COMPONENT_ID_TO_DEPLOYABLE_UNIT_ID_MAP + ":"
