...
public class ServiceComponent implements DeployedComponent, Serializable {
	...
	public ServiceComponent(ServiceDescriptorImpl serviceDescriptor)
			throws Exception {
	...
	private void enumerateSbbs(SbbID sbbId) throws Exception {
		...
		SbbDescriptor sbbDesc = SleeContainer.lookupFromJndi()
				.getSbbManagement().getSbbComponent(sbbId);
		if (sbbDesc == null) {
		...
		if (sbbIds != null) {
			for (int i = 0; i < sbbIds.length; i++) {
				...
				if (!sbbComponents.contains(nextSbbID)) {
					...
					logger
							.debug("Skipping recursive call to avoid infinite recursion SbbID : "
	...
	private void registerSbbUsageParameter(ServiceID serviceId, SbbID sbbId,
			String name, ObjectName sbbUsageParameter) {
		if (logger.isDebugEnabled()) {
			logger.debug("registerSbbUsageParameter: serviceId = " + serviceId
					+ " sbbId = " + sbbId + " name = " + name
					+ " sbbUsageParameter = " + sbbUsageParameter);
		}
		this.usageParameterObjectNames.put(Service.getUsageParametersPathName(
				serviceId, sbbId, name), sbbUsageParameter);
	}

	public ObjectName getUsageParameterObjectName(SbbID sbbId) {
		String key = Service.getUsageParametersPathName(serviceID, sbbId);
		logger
				.debug("getUsageParameterObjectName:  " + sbbId + " key = "
						+ key);
	...
	public ObjectName getUsageParameterObjectName(SbbID sbbId, String name)
		...
		if (!this.getAllUsageParameterNames().contains(name)) {
			...
			throw new UnrecognizedUsageParameterSetNameException(
					"Illegal arg - param name not found for this service "
							+ name);
		}

		if (this.usageParameterNames.get(sbbId.toString()) == null) {
			throw new InvalidArgumentException(
					"no usage parameter found for this sbb");
		}

		logger.debug("getUsageParameterObjectName:  " + sbbId + " name = "
				+ name + " key = " + key);
	...
	public void installUsageParameter(SbbID sbbId, String name)
		...
		try {
			...
			MobicentsSbbDescriptor sbbDescriptor = (MobicentsSbbDescriptor) SleeContainer
					.lookupFromJndi().getSbbManagement().getSbbComponent(sbbId);

			if (sbbDescriptor == null)
				throw new UnrecognizedSbbException("Unrecognized Sbb");

			if (this.usageParameterNames.get(sbbId.toString()) != null
					&& ((HashSet) this.usageParameterNames
							.get(sbbId.toString())).contains(name))
			...
			String pathName = Service.getUsageParametersPathName(this
					.getServiceID(), (SbbIDImpl) sbbDescriptor.getID(), name);

			// The service instance houses the actual usage parameter.
			try {

				Service.addUsageParameter((ServiceID) this.serviceID, pathName,
						usageParm);
			} catch (Exception ex) {
				throw new ManagementException("Service not found " + serviceID);
			}

			HashSet hset = (HashSet) this.usageParameterNames.get(sbbId
					.toString());
			if (hset == null) {
				hset = new HashSet();
	 ...
	 * @throws Exception
	 */
	public void installDefaultUsageParameters(
			MobicentsSbbDescriptor descriptor, Set sbbsChecked)
			throws Exception {
		...
		try {
			if (descriptor.getUsageParametersInterface() != null) {
				...
				InstalledUsageParameterSet usageParam = (InstalledUsageParameterSet) usageParm;
				String pathName = Service.getUsageParametersPathName(this
						.getServiceID(), (SbbID) descriptor.getID());

				if (logger.isDebugEnabled()) {
					logger.debug(" Putting usage parameter into table "
							+ pathName);
				}
				...
				Object[] objs = { this.serviceID, (SbbID) descriptor.getID(),
						null,
		...
		for (int i = 0; i < sbbs.length; i++) {
			if (!sbbsChecked.contains(sbbs[i])) {
				...
				MobicentsSbbDescriptor desc = (MobicentsSbbDescriptor) SleeContainer
						.lookupFromJndi().getSbbManagement().getSbbComponent(
								sbbs[i]);
	...
	public void removeUsageParameter(SbbID sbbId, String name)
		...
		if (!this.sbbComponents.contains(sbbId))
			throw new UnrecognizedSbbException(
					"SBB does not belong to this service " + sbbId);

		HashSet names = (HashSet) this.usageParameterNames
				.get(sbbId.toString());

		if (names == null)
			throw new InvalidArgumentException("UsageParameter name " + name
					+ " not found ");
	 ...
	 * @return
	 */
	public String[] getNamedUsageParameterSets(SbbID sbbId)
			throws InvalidArgumentException {

		HashSet paramSet = (HashSet) this.usageParameterNames.get(sbbId
				.toString());
		String[] retval = null;
		if (paramSet != null) {
			...
			throw new InvalidArgumentException(
					"No named usage parameters for this sbb " + sbbId);
	...
	public boolean equals(Object obj) {
		if (obj != null && obj.getClass() == getClass()) {
			return ((ServiceComponent)obj).serviceID.equals(this.serviceID);
		}
		else {
			return false;
		}
