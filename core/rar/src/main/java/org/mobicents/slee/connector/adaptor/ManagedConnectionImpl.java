 ...
 ***************************************************
 *
 * Created on Dec 5, 2004 ManagedConnectionImpl.java
 */
...
/**
 * Implementation of the ManagedConnection interface according to the
 * JCA1.0 contract
 * 
 * @author Tim
 */
public class ManagedConnectionImpl implements ManagedConnection, LocalTransaction {
    ...
    private void sendNotification(ConnectionEvent event) {
        log.debug("sendNotification() called with " + event + " eventID = " + event.getId());
    ...
    void fireEvent(Object event, EventTypeID eventType,
            ...
            throws ResourceException {
        log.debug("fireEvent() called:" + event + "," + eventType + "," + activityHandle + "," + address);
        if (destroyed)
            throw new IllegalStateException("Connection is destroyed!");
    ...
    /*
     * Actually send any queued events to the SLEE. To prevent too much network
     * traffic we send them using a method that takes the whole queue in one
     * remote call.
     */
