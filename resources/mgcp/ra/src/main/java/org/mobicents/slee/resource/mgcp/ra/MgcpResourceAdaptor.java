...
public class MgcpResourceAdaptor implements ResourceAdaptor, Serializable {
	...
	public void processMgcpCommandEvent(JainMgcpCommandEvent event) {
		...
		case Constants.CMD_AUDIT_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(auditConnection.getConnectionIdentifier(),
					event.getTransactionHandle(), "net.java.slee.resource.mgcp.AUDIT_CONNECTION", event);
			break;
		...
		case Constants.CMD_DELETE_CONNECTION:
			...
			if (deleteConnection.getConnectionIdentifier() != null) {
				processNonCreateConnectionMgcpEvent(deleteConnection.getConnectionIdentifier(), event
						.getTransactionHandle(), "net.java.slee.resource.mgcp.DELETE_CONNECTION", event);
			} else {
		...
		case Constants.CMD_MODIFY_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(modifyConnection.getConnectionIdentifier(), event
					.getTransactionHandle(), "net.java.slee.resource.mgcp.MODIFY_CONNECTION", event);
			break;
	...
	public void processMgcpResponseEvent(JainMgcpResponseEvent response, JainMgcpEvent command) {
		...
		case Constants.RESP_AUDIT_CONNECTION:
			processNonCreateConnectionMgcpEvent(((AuditConnection) command).getConnectionIdentifier(), response
					.getTransactionHandle(), "net.java.slee.resource.mgcp.AUDIT_CONNECTION_RESPONSE", response);
			break;
		...
		case Constants.RESP_CREATE_CONNECTION:
			...
			processNonCreateConnectionMgcpEvent(connectionIdentifier, response.getTransactionHandle(),
			...
			if (!response.getReturnCode().equals(ReturnCode.Transaction_Executed_Normally)) {
				...
				endActivity(mgcpActivityManager.getMgcpConnectionActivityHandle(connectionIdentifier, response
		...
		case Constants.RESP_DELETE_CONNECTION:
			...
			if (deleteConnection.getConnectionIdentifier() != null) {
				processNonCreateConnectionMgcpEvent(((DeleteConnection) command).getConnectionIdentifier(), response
		...
		case Constants.RESP_MODIFY_CONNECTION:
			processNonCreateConnectionMgcpEvent(((ModifyConnection) command).getConnectionIdentifier(), response
					.getTransactionHandle(), "net.java.slee.resource.mgcp.MODIFY_CONNECTION_RESPONSE", response);
			break;
	 ...
	 */
	private void processNonCreateConnectionMgcpEvent(ConnectionIdentifier connectionIdentifier, int transactionHandle,
			String eventName, Object eventObject) {
	...
	public void processTimeout(JainMgcpCommandEvent event) {
		...
		case Constants.CMD_AUDIT_CONNECTION:
			handle = mgcpActivityManager.getMgcpConnectionActivityHandle(((AuditConnection) event)
					.getConnectionIdentifier(), event.getTransactionHandle());
			break;
