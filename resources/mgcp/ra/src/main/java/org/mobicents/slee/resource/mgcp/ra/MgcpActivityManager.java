...
public class MgcpActivityManager {
    ...
    public MgcpConnectionActivityHandle putMgcpConnectionActivity(MgcpConnectionActivityImpl activity) {
    	...
    	if (activity.getConnectionIdentifier() != null) {
    		...
    		if (logger.isDebugEnabled()) {
        		logger.debug("created mapping between connection identifier "+activity.getConnectionIdentifier()+" and activity handle "+handle+")");
        	}
    	}
    	else if (activity.getTransactionHandle() != null) {
    		transactionHandle2ActivityHandleMap.put(activity.getTransactionHandle(), handle);
    		if (logger.isDebugEnabled()) {
        		logger.debug("created temp mapping between transaction handle "+activity.getTransactionHandle()+" and activity handle "+handle+")");
        	}
    	}
    ...
    public MgcpConnectionActivityHandle updateMgcpConnectionActivity(int transactionHandle,ConnectionIdentifier connectionIdentifier,EndpointIdentifier endpointIdentifier) {
    	...
    	if (connectionIdentifier != null) {
        	...
        	if (handle != null) {
        		...
        		if (activity == null) {
        			logger.warn("update of MgcpConnectionActivity failed, activity for connectionIdentifier "+connectionIdentifier+" not found");
        		}
        		else {
        		...
        		if (logger.isDebugEnabled()) {
            		logger.debug("update of MgcpConnectionActivity failed, transactionHandle "+transactionHandle+" not found");
            	}
        	}
		}
		else {			
