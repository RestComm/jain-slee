...
public class SipResourceAdaptor implements SipListener, ResourceAdaptor,
		...
		for (int i = 0; i < listeningPoints.length; i++) {
			...
			for (int k = 0; k < 10; k++) {
				try {
					...
					log
							.error("Object in use -- retrying to delete listening point",ex);
					try {
						Thread.sleep(100);
					} catch (Exception e) {
	...
	public void activityEnded(ActivityHandle handle) {
		...
		WrapperSuperInterface activity = (WrapperSuperInterface) this.activities.remove(handle);
		if (activity != null) {
			activity.cleanup();
		}
	}
	...
	private void processCancelRequest(ServerTransaction st,
		...
		if (inviteSTW != null) {
			...
			if ((inviteSTW.getState() == TransactionState.TERMINATED)
				...
				log.error("Invite transaction has been found in state other than proceeding, final response sent, sending BAD_REQUEST");
			...
			if (inviteSTW.getDialog() != null) {
				...
				fireEvent(REW, SAH, eventID, new Address(
	...
	private void processNotCancelRequest(ServerTransaction st,
		...
		if (!fireEvent(REW, SAH, eventIdCache.getEventId(eventLookup,REW.getRequest(),inDialog),new Address(
			...
			sendErrorResponse(req.getServerTransaction(),req.getRequest(),
						Response.SERVER_INTERNAL_ERROR,"Failed to deliver request event to JAIN SLEE container");
		}
	}
	...
	public void processResponse(ResponseEvent resp) {
		...
		fireEvent(REW, SAH, eventID,new Address(
				AddressPlan.SIP, ((FromHeader) resp.getResponse().getHeader(
			...
			if (!arg0.isServerTransaction()) {
				...
				if (t.getApplicationData() == null
					...
					fireEvent(tew, handle, eventIdCache.getTimeoutEventId(eventLookup, inDialog),new Address(
							AddressPlan.SIP, ((FromHeader) ctw.getRequest().getHeader(
									FromHeader.NAME)).getAddress().toString()));
				}
				...
				if (sendClientResponse) {
					try {
						...
						fireEvent(REW, da.getActivityHandle(),
								eventIdCache.getEventId(eventLookup, response,inDialog),new Address(
										AddressPlan.SIP, ((FromHeader) response.getHeader(
												FromHeader.NAME)).getAddress().toString()));
		...
		if (t.getApplicationData() != null) {
			...
			if (t.getDialog() != null && t.getDialog().getApplicationData() instanceof DialogWrapper) {
				...
				if (wsi instanceof ServerTransactionWrapper) {
					...
					log.error("Unknown type "+wsi.getClass()+" of SIP Transaction, can't remove from dialog wrapper");
				}
			}
	...
	*/
	private boolean fireEvent(Object event, ActivityHandle handle,
			int eventID, Address address, boolean useFiltering) {
