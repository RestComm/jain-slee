...
public abstract class DialogCreationEarlyCancelClientTestSbb extends
		SuperSipRaTestSbb {
	...
	public void onNotify(javax.sip.RequestEvent event,
		...
		try {
			...
			Address contactAddress = ((ContactHeader) event.getRequest()
					.getHeader(ContactHeader.NAME)).getAddress();
			...
			logger.info("-----------------> ROUTE ADDRESS:" + rh_Address
					+ "\nHEADER:" + rh);
			...
			DialogActivity da = (DialogActivity) this.fp.getNewDialog(ctx);
			ActivityContextInterface d_aci = this.acif
					.getActivityContextInterface(da);
			d_aci.attach(this.getSbbContext().getSbbLocalObject());
			ctx.sendRequest();
	...
	public void onRinging(ResponseEvent event, ActivityContextInterface aci) {
		// aci.getActivity == dialog
		try {

			//Dialog d = ((Dialog) aci.getActivity());

			
			if (event.getResponse().getStatusCode() == 180) {
				
				Request cancelRequest=event.getClientTransaction().createCancel();
				ClientTransaction cancelCTX=this.fp.getNewClientTransaction(cancelRequest);
				this.acif.getActivityContextInterface(cancelCTX).attach(this.getSbbContext().getSbbLocalObject());
