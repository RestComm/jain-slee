package org.csapi.fw.fw_service.integrity;

import org.omg.PortableServer.POA;

/**
 *	Generated from IDL interface "IpFwHeartBeat"
 *	@author JacORB IDL compiler V 2.1, 16-Feb-2004
 */

public class IpFwHeartBeatPOATie
	extends IpFwHeartBeatPOA
{
	private IpFwHeartBeatOperations _delegate;

	private POA _poa;
	public IpFwHeartBeatPOATie(IpFwHeartBeatOperations delegate)
	{
		_delegate = delegate;
	}
	public IpFwHeartBeatPOATie(IpFwHeartBeatOperations delegate, POA poa)
	{
		_delegate = delegate;
		_poa = poa;
	}
	public org.csapi.fw.fw_service.integrity.IpFwHeartBeat _this()
	{
		return org.csapi.fw.fw_service.integrity.IpFwHeartBeatHelper.narrow(_this_object());
	}
	public org.csapi.fw.fw_service.integrity.IpFwHeartBeat _this(org.omg.CORBA.ORB orb)
	{
		return org.csapi.fw.fw_service.integrity.IpFwHeartBeatHelper.narrow(_this_object(orb));
	}
	public IpFwHeartBeatOperations _delegate()
	{
		return _delegate;
	}
	public void _delegate(IpFwHeartBeatOperations delegate)
	{
		_delegate = delegate;
	}
	public POA _default_POA()
	{
		if (_poa != null)
		{
			return _poa;
		}
		else
		{
			return super._default_POA();
		}
	}
	public void pulse() throws org.csapi.TpCommonExceptions
	{
_delegate.pulse();
	}

}
