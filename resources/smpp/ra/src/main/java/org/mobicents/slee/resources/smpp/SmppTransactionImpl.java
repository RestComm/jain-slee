...
public class SmppTransactionImpl implements SmppTransaction {
	...
	protected void sendGenericNack() {
		...
		try {
			...
			this.smppRA.tracer.severe("VersionException while sending GenericNack SMPP Response", e);
		} catch (BadCommandIDException e) {
			this.smppRA.tracer.severe("BadCommandIDException while sending GenericNack SMPP Response", e);
		} catch (IOException e) {
			this.smppRA.tracer.severe("IOException while sending GenericNack SMPP Response", e);
		} catch (SmppDateFormatException e) {
			this.smppRA.tracer.severe("SmppDateFormatException while sending GenericNack SMPP Response", e);
		}
	}
