...
public abstract class DialogSetupFailed_C2TestSbb extends BaseTCKSbb {
	...
	public void onStartServiceEvent(
		...
		try {
			...
			ActivityContextInterface inviteACI = SIPACIF
					.getActivityContextInterface(CTInvite);
			SbbLocalObject SLO = this.sbbContext.getSbbLocalObject();
			dialACI.attach(SLO);
			inviteACI.attach(SLO);
		} catch (SipException e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
	...
	public void onInformation(javax.sip.ResponseEvent event,
		...
		if (resp.getStatusCode() == 101
			...
			try {
				logger
						.info("\n----------------------------\nSending CANCEL\n-------------------------------\n");
				Request cancel = getInviteCTX().createCancel();
				ClientTransaction ctx = fp.getSipProvider()
	...
	public void onDialogSetupFailed(javax.sip.ResponseEvent event,
		...
		if (getSetupEarlyReceived() && getSetupFailedReceived()) {
			try {
	...
	private ClientTransaction getInviteCTX() {
		...
		for (int i=0;i<acis.length;i++) {
			ActivityContextInterface aci=acis[i];
			if (aci.getActivity() instanceof ClientTransaction) {
