...
import org.mobicents.slee.resource.sip.SipFactoryProvider; //import org.mobicents.slee.resource.sip.SipActivityContextInterfaceFactory;
...
public abstract class DialogTerminated_3TestSbb extends BaseTCKSbb {
	...
	public abstract boolean getDialogTerminationReceived();

	public abstract void setDialogTerminationReceived(boolean flag);
	...
	public void onDialogSetupEarly(javax.sip.ResponseEvent event,
		...
		try {
			...
			ContactHeader contact = headerFactory
					.createContactHeader(((ToHeader) event.getResponse()
	...
	public void onDialogTerminationRequest(javax.sip.RequestEvent event,
		...
		logger
				.info("=================  GOT DIALOG TERMINATION REQUEST EVENT =================== \n"
						+ event + "\n==================");
		setDialogTerminationReceived(true);
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		logger
				.info("=================  GOT DIALOG TERMINATED EVENT =================== \n"
						+ event + "\n==================");
		try {
			if (getSetupConfirmedReceived() && getSetupEarlyReceived()
					&& getDialogTerminationReceived()) {
				setResultPassed(" == RECEIVED SetupEarly("
						+ getSetupEarlyReceived() + ")SetupConfirmed("
						+ getSetupConfirmedReceived()
						+ "), TerminationRequest("
						+ getDialogTerminationReceived()
						+ ") and Dialog Terminated(true) events in order ==");
			} else {
				setResultFailed(" == RECEIVED SetupEarly("
						+ getSetupEarlyReceived() + ")SetupConfirmed("
						+ getSetupConfirmedReceived()
						+ "), TerminationRequest("
						+ getDialogTerminationReceived()
	...
	public void onTimeEvent(javax.slee.facilities.TimerEvent event,
		...
		if (!(getSetupEarlyReceived() && getSetupConfirmedReceived()
				&& getDialogTerminatedReceived() && getDialogTerminationReceived()))
			try {
				setResultFailed("DIDNT RECEIVE EVENT WITHIN 20s\nsetupEarlyReceived:"
						+ getSetupEarlyReceived()
						+ "\nsetupConfirmedReceived:"
						+ getSetupConfirmedReceived()
						+ "\ndialogTerminationRequest:"
						+ getDialogTerminationReceived()
						+ "\ndialogTerminated:"
						+ getDialogTerminatedReceived());
			} catch (Exception e) {
