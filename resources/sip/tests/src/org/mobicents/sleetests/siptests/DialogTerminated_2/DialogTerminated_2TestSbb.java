	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		try {
			if (getSetupEarlyReceived()&&getSetupFailedReceived()) {
				...
				setResultFailed(" == RECEIVED SetupEarly("+getSetupEarlyReceived()+")-SetupFailed("+getSetupFailedReceived()+") and Terminated(true) events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
