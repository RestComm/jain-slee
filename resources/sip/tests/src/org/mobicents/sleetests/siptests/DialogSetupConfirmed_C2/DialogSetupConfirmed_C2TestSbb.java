...
public abstract class DialogSetupConfirmed_C2TestSbb extends BaseTCKSbb {
	...
	public void onDialogSetupEarly(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT DIALOG SetupEarly EVENT["+(++earlyCount)+"] =================== \n"
						+ event + "\n==================");
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		logger
				.info("=================  GOT DIALOG SetupConfirmed EVENT["+(++confirmedCount)+"] =================== \n"
						+ event + "\n==================");
		try {
			if(getSetupEarlyReceived())
				setResultPassed("GOT setupEarlySetup("+getSetupEarlyReceived()+") and setupConfirmed("+getSetupConfirmedReceived()+")!!!");
			else
				setResultFailed("GOT setupEarlySetup("+getSetupEarlyReceived()+") and setupConfirmed("+getSetupConfirmedReceived()+")!!!");
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
		}
