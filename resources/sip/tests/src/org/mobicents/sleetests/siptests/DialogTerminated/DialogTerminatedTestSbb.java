...
public abstract class DialogTerminatedTestSbb extends BaseTCKSbb {
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		try {
			if (getSetupFailedReceived()) {
				setResultPassed(" == RECEIVED setupFailed("+getSetupFailedReceived()+") and Terminated(true) events in order ==");
			} else {
				setResultFailed(" == DID NOT RECEIVE setupFailed("+getSetupFailedReceived()+") and Terminated(true) events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
	...
	public void onTimeEvent(javax.slee.facilities.TimerEvent event,
			ActivityContextInterface aci) {
		if (!getDialogTerminatedReceived() || !getSetupFailedReceived())
			try {
