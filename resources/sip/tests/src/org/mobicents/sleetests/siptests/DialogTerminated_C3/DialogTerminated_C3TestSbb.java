...
public abstract class DialogTerminated_C3TestSbb extends BaseTCKSbb {
	...
	private void onInformation(javax.sip.ResponseEvent event,
			ActivityContextInterface aci) {
	...
	public void onDialogSetupConfirmed(javax.sip.ResponseEvent event,
		...
		try {
			event.getDialog().sendAck(event.getDialog().createAck(((CSeqHeader)event.getResponse().getHeader(CSeqHeader.NAME)).getSeqNumber()));
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
		}
	...
	public void onDialogTerminated(javax.sip.DialogTerminatedEvent event,
		...
		try {
			if (getSetupConfirmedReceived() && getSetupEarlyReceived()) {
				setResultPassed(" == RECEIVED setupEarly("+getSetupEarlyReceived()+"), setupConfirmed("+getSetupConfirmedReceived()+") and Terminated("+true+") events in order ==");
			} else {
				setResultFailed(" == DID NOT RECEIVE setupEarly("+getSetupEarlyReceived()+"), setupConfirmed("+getSetupConfirmedReceived()+") and Terminated("+true+") events in order ==");
			}
		} catch (Exception e) {
			TCKSbbUtils.handleException(e);
			e.printStackTrace();
