SMPP Resource Adaptor ussage example
-------------------------------------
This application receives short message and sends received text back
-------------------------------------

Requirements:
-------------------------------------
1) SMSc simulator (http://www.seleniumsoftware.com/downloads.html).

Running
-------------------------------------
1. Download SMPPSimulator
2) Edit SMPPSim config (smppsim_home/conf/smppsim.props) as follows:
...
3) switch to SMPPSim home directory and start simulator usig startsmppsim command
4) install SMPP Resource Adaptor
5) install SMPP Demo application
6) run web browser and point it to http://localhost:88
7) open page "Inject an MO message"
8) change "destination_addr" field vale to 0020
9) Press on Submit button

the message will be delivered to application and application will reply with same message
see SMPPSim log for details.
