...
public class ParameterSetListPanel extends Composite {
  ...
  private void onResetAllUsageParameters() {
    ServerCallback callback = new ServerCallback(this) {
      public void onSuccess(Object result) {
        Logger.info("All parameters reset for SBB " + getSelectedSbbInfo().getName() + " within service " + getSelectedServiceInfo().getName());
      }
    };
    Logger.info("Resetting all parameters " + getSelectedServiceInfo().getID() + " - " + getSelectedSbbInfo().getID());
    usageService.resetAllUsageParameters(getSelectedServiceInfo().getID(), getSelectedSbbInfo().getID(), callback);
  }
  ...
  private void onCreateParameterSet(final String name) {
    ...
    ServerCallback callback = new ServerCallback(this) {
      public void onSuccess(Object result) {
        ...
        Logger.info("Parameter set " + name + " created for SBB " + getSelectedSbbInfo().getName() + " within service " + getSelectedServiceInfo().getName());
      }
    };

    usageService.createUsageParameterSet(getSelectedServiceInfo().getID(), getSelectedSbbInfo().getID(), name, callback);
  }
  ...
  private void refreshServiceListBox() {
    ...
    componentsService.getComponentInfos(new ComponentTypeInfo(ComponentTypeInfo.SERVICE, 0), serviceListCallback);
  }
  ...
  private void refreshSbbListBox() {
    ...
    servicesService.getSbbsWithinService(getSelectedServiceInfo().getID(), sbbListCallback);
  }
  ...
  private void refreshParameterSets() {
    ...
    usageService.getParameterSets(serviceInfo.getID(), sbbInfo.getID(), callback);
  ...
  private void onParameterSet(String name) {
    ParameterSetPanel parameterSetPanel = new ParameterSetPanel(browseContainer, getSelectedServiceInfo(), getSelectedSbbInfo(), name);
    if (name.length() == 0)
  ...
  private void onRemoveParameterSet(final String name) {
    ...
    ServerCallback callback = new ServerCallback(this) {
      public void onSuccess(Object result) {
        Logger.info("Parameter set " + name + " removed for SBB " + getSelectedSbbInfo().getName() + " within service " + getSelectedServiceInfo().getName());
        refreshParameterSets();
      }
    };

    usageService.removeUsageParameterSet(getSelectedServiceInfo().getID(), getSelectedSbbInfo().getID(), name, callback);
  }
