 ...
 */
public class RaTypeXMLSubmenu implements IObjectActionDelegate, IMenuCreator {

  public RaTypeXMLSubmenu() {
    super();
  }
  ...
  private void fillMenu(Menu menu) {
    createRaTypeMenus(menu);
  }

  private void createRaTypeMenus(Menu parent) {
    ...
    if (obj instanceof IFile) {
      try {
        ResourceAdaptorTypeJarXML raTypeJarXML = new ResourceAdaptorTypeJarXML((IFile) obj);
        ResourceAdaptorTypeXML raTypes[] = raTypeJarXML.getResourceAdaptorTypes();

        for (int i = 0; i < raTypes.length; i++) {

          Menu child = new Menu(parent);
          MenuItem item = new MenuItem(parent, SWT.CASCADE);
          item.setText(raTypes[i].getName() + ", " + raTypes[i].getVersion() + ", " + raTypes[i].getVendor());
          item.setMenu(child);
  ...
  private class EventsSelectionListener extends SelectionAdapter {
    public void widgetSelected(SelectionEvent e) {
      ...
      EditRaTypeEventsAction action = new EditRaTypeEventsAction(parentItem.getText());
  ...
  private class IdentitySelectionListener extends SelectionAdapter {
    public void widgetSelected(SelectionEvent e) {
      ...
      Menu parent = item.getParent();
      MenuItem parentItem = parent.getParentItem();
      EditRaTypeIdentityAction action = new EditRaTypeIdentityAction(parentItem.getText());
      action.selectionChanged(null, selection);
      action.run(null);			
    }
  }
  ...
  private class DeleteSelectionListener extends SelectionAdapter {
    public void widgetSelected(SelectionEvent e) {
      ...
      DeleteRaTypeAction action = new DeleteRaTypeAction(parentItem.getText());
      action.selectionChanged(null, selection);
      action.run(null);			
    }	
  }
