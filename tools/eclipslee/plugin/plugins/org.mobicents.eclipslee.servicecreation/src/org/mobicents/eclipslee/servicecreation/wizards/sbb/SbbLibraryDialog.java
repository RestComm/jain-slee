...
import org.mobicents.eclipslee.servicecreation.ui.SbbLibraryPanel;
...
public class SbbLibraryDialog extends Dialog {
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    panel = new SbbLibraryPanel(composite, 0);
  ...
  private void setLibraries(LibraryRefXML[] profiles) {
    selectedLibraries = new HashMap[profiles.length];

    for (int i = 0; i < profiles.length; i++) {
      // Name, Vendor, Version, XML (is set in panel)
      selectedLibraries[i] = new HashMap();
      selectedLibraries[i].put("Name", profiles[i].getName());
      selectedLibraries[i].put("Vendor", profiles[i].getVendor());
      selectedLibraries[i].put("Version", profiles[i].getVersion());			
    }
  }

  private SbbLibraryPanel panel;
