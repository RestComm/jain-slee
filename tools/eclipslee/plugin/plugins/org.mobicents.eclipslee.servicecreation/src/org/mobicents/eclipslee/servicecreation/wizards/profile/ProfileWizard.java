...
public final class ProfileWizard extends BaseWizard {
  ...
  public void addPages() {
    super.addPages();
    // Libraries
    profileLibrariesPage = new ProfileLibraryPage(WIZARD_TITLE);
    addPage(profileLibrariesPage);
    // CMP
    profileCmpPage = new ProfileCMPPage(WIZARD_TITLE);
    addPage(profileCmpPage);
  }

  public boolean performFinish() {
    createProfileAbstractClass = profileCmpPage.getCreateAbstractClass();

    profileLibraries = profileLibrariesPage.getSelectedLibraries();
  ...
  public void doFinish(IProgressMonitor monitor) throws CoreException {
    ...
    profileCmpFields = profileCmpPage.getCMPFields();
    ...
    for (int i = 0; i < profileCmpFields.length; i++) {
      HashMap cmpData = profileCmpFields[i];
    ...
    if (visibleFields.size() < profileCmpFields.length)
    ...
    String accessors[] = CMPUtil.getAccessors(profileCmpFields);
    String tmp = "";
    ...
    try {
      ...
      ProfileSpecJarXML profileSpecJarXML = profileSpecJarFile.exists() ? new ProfileSpecJarXML(profileSpecJarFile) : new ProfileSpecJarXML();
      ProfileSpecXML profileSpecXML = profileSpecJarXML.addProfileSpec(getComponentName(), getComponentVendor(), getComponentVersion(), getComponentDescription());
      ...
      for (int i = 0; i < profileCmpFields.length; i++) {
        ...
        if (indexed) {
          ...
          try {
            profileSpecXML.addIndexedAttribute((String) cmpData.get("Name"), unique);
      ...
      FileUtil.createFromInputStream(resourceFolder, new Path(xmlFilename), profileSpecJarXML.getInputStreamFromXML(), monitor);
      monitor.worked(1);
  ...
  private ProfileCMPPage profileCmpPage;
  private ProfileLibraryPage profileLibrariesPage;

  private HashMap[] profileCmpFields;
  private HashMap[] profileLibraries;
