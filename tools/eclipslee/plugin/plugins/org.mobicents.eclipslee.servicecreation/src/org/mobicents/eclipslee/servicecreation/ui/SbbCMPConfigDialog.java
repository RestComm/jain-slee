...
public class SbbCMPConfigDialog extends Dialog implements ModifyListener, SelectionListener {
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    // Serializable Java Object of primitive
    ...
    aciButton = new Button(composite, SWT.RADIO);
    aciButton.setText("This CMP field holds an Activity Context reference");
    data = new GridData(GridData.FILL_HORIZONTAL);
    aciButton.setLayoutData(data);
    aciButton.addSelectionListener(this);

    // Event Context reference
    evcButton = new Button(composite, SWT.RADIO);
    evcButton.setText("This CMP field holds an Event Context reference");
    data = new GridData(GridData.FILL_HORIZONTAL);
    evcButton.setLayoutData(data);
    evcButton.addSelectionListener(this);

    // Profile Local Object reference
    ploButton = new Button(composite, SWT.RADIO);
    ploButton.setText("This CMP field holds an ProfileLocalObject reference");
    data = new GridData(GridData.FILL_HORIZONTAL);
    ploButton.setLayoutData(data);
    ploButton.addSelectionListener(this);
  ...
  public void okPressed() {
    ...
    if (isSbbLocalObject()) {
      // TODO: Fetch custom name?
      type = "javax.slee.SbbLocalObject";
    }
    else if (isActivityContextInterface()) {
      // TODO: Fetch custom name?
      type = "javax.slee.ActivityContextInterface";
    }
    else if (isEventContext()) {
      type = "javax.slee.EventContext";
    }
    else if (isProfileLocalObject()) {
      // TODO: Fetch custom name?
      type = "javax.slee.profile.ProfileLocalObject";
    }
  ...
  private void validation() {
    ...
    try {
      ...
      boolean sbbLO = sbbButton.getSelection();
      boolean aci = aciButton.getSelection();
      boolean eventCtx = evcButton.getSelection();
      boolean profileLO = ploButton.getSelection();
      ...
      if (primitive) {
        // good if we have a name and type
        okEnabled = name.length() > 0 && type.length() > 0;
      }
      else if (sbbLO) {
        // we are good if we have a name, an sbb selected and a scoped name set
        okEnabled = name.length() > 0 && sname.length() > 0 && sbbCombo.getSelectionIndex() >= 0;
      }
      else if (aci || eventCtx || profileLO) {
        // we only need the name here
        okEnabled = name.length() > 0;
      }
  ...
  private void initialize() {
    ...
    if (project == null) {
      ...
      Vector<SbbXML> sbbs = new Vector<SbbXML>();
    ...
    if (type.equals("javax.slee.SbbLocalObject")) {
      sbbButton.setSelection(true);
    }
    else if (type.equals("javax.slee.ActivityContextInterface")) {
      aciButton.setSelection(true);
    }
    else if (type.equals("javax.slee.EventContext")) {
      evcButton.setSelection(true);
    }
    else if (type.equals("javax.slee.profile.ProfileLocalObject")) {
      ploButton.setSelection(true);
    }
    else {
  ...
  public boolean isActivityContextInterface() {
    return aciButton.getSelection();
  }

  public boolean isEventContext() {
    return evcButton.getSelection();
  }

  public boolean isProfileLocalObject() {
    return ploButton.getSelection();
  }
  ...
  private Button aciButton;
  private Button evcButton;
  private Button ploButton;
