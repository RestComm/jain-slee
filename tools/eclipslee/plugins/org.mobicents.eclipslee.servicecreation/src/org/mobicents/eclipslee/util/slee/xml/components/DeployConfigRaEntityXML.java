 ...
 */
public class DeployConfigRaEntityXML extends DTDXML {

	protected DeployConfigRaEntityXML(Document document, Element root, DTDHandler dtd) {
		super(document, root, dtd);
	}
	
  public String getResourceAdaptorId() {
    return getRoot().getAttribute("resource-adaptor-id");   
  }
  
  public void setResourceAdaptorId(String resourceAdaptorId) {
    getRoot().setAttribute("resource-adaptor-id", resourceAdaptorId);
  }
  
  public String getEntityName() {
    return getRoot().getAttribute("entity-name");   
  }
  
  public void setEntityName(String entityName) {
    getRoot().setAttribute("entity-name", entityName);
  }
  
  public DeployConfigPropertyXML addProperty(String name, String type, String value) {
    Element properties = getChild(getRoot(), "properties");

    if (properties == null)
      properties = addElement(getRoot(), "properties");

    Element property = addElement(properties, "property");
    property.setAttribute("name", name);
    property.setAttribute("type", name);

    if (value != null) {
      property.setAttribute("value", name);
    }

    return new DeployConfigPropertyXML(document, property, dtd);
  }
