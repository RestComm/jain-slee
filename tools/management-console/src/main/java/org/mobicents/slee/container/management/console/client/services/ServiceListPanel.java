...
public class ServiceListPanel extends Composite {
	...
	private void refreshTable() {
		...
		for (int i = 0; i < serviceInfoHeaders.length; i++) {
			...
			// COLUMN_ICON
			serviceListPanel.setCell(i, COLUMN_ICON, new Image(
					"images/services.service."
							+ serviceInfoHeader.getServiceStateInfo()
									.getState().toLowerCase() + ".gif"));

			// COLUMN_NAME
			Hyperlink nameLink = new Hyperlink(serviceInfoHeader.getName(),
					serviceInfoHeader.getName());
			...
			// COLUMN_STATE
			serviceListPanel.setCellText(i, COLUMN_STATE, serviceInfoHeader
					.getServiceStateInfo().getState());
			...
			if (serviceInfoHeader.getServiceStateInfo().getState().equals(ServiceStateInfo.INACTIVE)) {
				...
				Hyperlink deactivateLink = new Hyperlink("deactivate",
						"deactivate " + serviceInfoHeader.getName());
