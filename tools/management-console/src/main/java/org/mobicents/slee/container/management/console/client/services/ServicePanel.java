...
public class ServicePanel extends Composite implements ComponentNameClickListener {
	...
	private void refreshServicePropertiesPanel(ServiceInfo serviceInfo) {
		...
		ComponentPropertiesPanel componentPropertiesPanel = new ComponentPropertiesPanel(
				serviceInfo);
		...
		servicePropertiesPanel.add("Address profile", serviceInfo
				.getAddressProfileTable());
		servicePropertiesPanel.add("Resource profile", serviceInfo
	...
	private void refreshSbbs(ComponentInfo[] sbbInfos) {
		...
		for (int i = 0; i < sbbInfos.length; i++) {
			...
			Hyperlink sbbNameLink = new Hyperlink(sbbInfos[i].getName(),
					sbbInfos[i].getName());
