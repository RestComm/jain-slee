...
public class DeployableUnitInfoUtils {
	...
	static public DeployableUnitInfo toDeployableUnitInfo(
			DeployableUnitDescriptor deployableUnitDescriptor) {
		
		String id;
		if (deployableUnitDescriptor instanceof DeployableUnitDescriptorEx)
			id = toString(((DeployableUnitDescriptorEx) deployableUnitDescriptor)
					.getDeployableUnitID());
		else
			id = null;

		return new DeployableUnitInfo(ComponentInfoUtils
				.toStringArray(deployableUnitDescriptor.getComponents()),
				deployableUnitDescriptor.getDeploymentDate(),
				deployableUnitDescriptor.getURL(),
				getDeployableUnitName(deployableUnitDescriptor), id);
	}

	static public DeployableUnitInfo[] toDeployableUnitInfos(
	...
	static private String getDeployableUnitName(
			DeployableUnitDescriptor deployableUnitDescriptor) {
