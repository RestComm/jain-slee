...
public class DeployableUnitsServiceImpl extends RemoteServiceServlet implements
	...
	public DeployableUnitInfo[] getDeployableUnits()
			throws ManagementConsoleException {
		DeployableUnitInfo[] deployableUnitInfos = DeployableUnitInfoUtils
				.toDeployableUnitInfos(deploymentMBeanUtils
						.getDeployableUnitDescriptors());

		return deployableUnitInfos;
	}

	public DeployableUnitInfo[] searchDeployableUnits(String text)
			throws ManagementConsoleException {
		if (text == null || text.trim().equals(""))
			return null;

		String lowerCaseText = text.toLowerCase();
		DeployableUnitDescriptor[] allDeployableUnitDescriptor = deploymentMBeanUtils
				.getDeployableUnitDescriptors();
		...
		for (int i = 0; i < allDeployableUnitDescriptor.length; i++) {
			...
			if (deployableUnitDescriptor.getURL() != null
					&& deployableUnitDescriptor.getURL().toLowerCase().indexOf(
							lowerCaseText) > -1)
				resultDeployableUnitDescriptorArrayList
						.add(deployableUnitDescriptor);
		}

		if (resultDeployableUnitDescriptorArrayList.size() == 0)
			return null;

		resultDeployableUnitDescriptors = new DeployableUnitDescriptor[resultDeployableUnitDescriptorArrayList
				.size()];
		resultDeployableUnitDescriptors = resultDeployableUnitDescriptorArrayList
				.toArray(resultDeployableUnitDescriptors);

		resultDeployableUnitInfos = DeployableUnitInfoUtils
				.toDeployableUnitInfos(resultDeployableUnitDescriptors);

		return resultDeployableUnitInfos;
	}

	public void uninstall(String id) throws ManagementConsoleException {
		DeployableUnitID deployableUnitID = managementConsole
				.getDeployableUnitIDMap().get(id);
