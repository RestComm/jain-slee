 ...
 * 
 */
public class ServicesServiceImpl extends RemoteServiceServlet implements
		ServicesService {
	...
	private DeploymentMBeanUtils deploymentMBeanUtils = sleeConnection
			.getSleeManagementMBeanUtils().getDeploymentMBeanUtils();

	private ServiceManagementMBeanUtils serviceManagementMBeanUtils = sleeConnection
			.getSleeManagementMBeanUtils().getServiceManagementMBeanUtils();

	public ServiceInfoHeader[] getServiceInfoHeaders()
			throws ManagementConsoleException {

		ServiceID[] serviceIDs = deploymentMBeanUtils.getServices();
		ArrayList<ServiceInfoHeader> serviceInfoHeadersArrayList = new ArrayList<ServiceInfoHeader>(
				serviceIDs.length);
		ServiceInfoHeader[] serviceInfoHeaders = new ServiceInfoHeader[serviceIDs.length];

		for (int i = 0; i < serviceIDs.length; i++) {
			ServiceDescriptor serviceDescriptor = (ServiceDescriptor) deploymentMBeanUtils
					.getDescriptor(serviceIDs[i]);

			ServiceState serviceState = serviceManagementMBeanUtils
					.getState(serviceIDs[i]);

			ServiceInfoHeader serviceInfoHeader = ServiceInfoUtils
					.toServiceInfoHeader(serviceDescriptor, serviceState);

			serviceInfoHeadersArrayList.add(serviceInfoHeader);
		}

		serviceInfoHeaders = serviceInfoHeadersArrayList
				.toArray(serviceInfoHeaders);
	...
	private SbbID[] getUsedSbbs(SbbID sbbID) throws ManagementConsoleException {
		SbbDescriptor sbbDescriptor = (SbbDescriptor) deploymentMBeanUtils
				.getDescriptor(sbbID);

		return sbbDescriptor.getSbbs();
	}

	private void addUsedSbbs(ArrayList<SbbID> resultSbbIDs, SbbID[] sbbIDs)
			throws ManagementConsoleException {
		if (sbbIDs == null)
			return;
	...
	public ComponentInfo[] getSbbsWithinService(String strServiceID)
			throws ManagementConsoleException {

		ServiceID serviceID = (ServiceID) managementConsole.getComponentIDMap()
				.get(strServiceID);
		...
		addUsedSbbs(resultSbbIDArrayList, new SbbID[] { serviceDescriptor
				.getRootSbb() });
		...
		ComponentDescriptor[] referringSbbComponentDescriptors = deploymentMBeanUtils
				.getDescriptors(referringSbbIDs);

		return ComponentInfoUtils
				.toComponentInfos(referringSbbComponentDescriptors);
	}
}
