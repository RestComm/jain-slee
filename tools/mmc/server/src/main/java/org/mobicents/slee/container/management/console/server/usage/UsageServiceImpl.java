 ...
 * 
 */
public class UsageServiceImpl extends RemoteServiceServlet implements
		UsageService {
	...
	private ServiceUsageMBeanUtils getServiceUsageMBeanUtils(ServiceID serviceID)
			throws ManagementConsoleException {
		return sleeConnection.getSleeManagementMBeanUtils()
				.getServiceManagementMBeanUtils().getServiceUsageMBeanUtils(
						serviceID);
	}

	public String[] getParameterSets(String strServiceID, String strSbbID)
			throws ManagementConsoleException {
		...
		SbbID sbbID = (SbbID) managementConsole.getComponentIDMap().get(
				strSbbID);
		...
		try {
			...
			throw new ManagementConsoleException(
					"No SBB usage parameter interface defined for SBB "
							+ sbbID.toString());
		}
		...
		try {
			namedParameterSet = serviceUsageMBeanUtils
					.getUsageParameterSets(sbbID);
		} catch (Exception e) {
		}
	...
	public void createUsageParameterSet(String strServiceID, String strSbbID,
			String parameterSet) throws ManagementConsoleException {
	...
	public void removeUsageParameterSet(String strServiceID, String strSbbID,
			String parameterSet) throws ManagementConsoleException {
		ServiceID serviceID = (ServiceID) managementConsole.getComponentIDMap()
				.get(strServiceID);
	...
	public void resetAllUsageParameters(String strServiceID, String strSbbID)
			throws ManagementConsoleException {
		ServiceID serviceID = (ServiceID) managementConsole.getComponentIDMap()
				.get(strServiceID);
	...
	public SBBUsageParameterInfo[] getSBBUsageParameters(String strServiceID,
			String strSbbID, String parameterSet)
			throws ManagementConsoleException {
		ServiceID serviceID = (ServiceID) managementConsole.getComponentIDMap()
				.get(strServiceID);
		...
		if (parameterSet.length() == 0) {
			sbbUsageMBeanUtils = serviceUsageMBeanUtils
					.getSbbUsageMBeanUtils(sbbID);
		} else {
			sbbUsageMBeanUtils = serviceUsageMBeanUtils.getSbbUsageMBeanUtils(
					sbbID, parameterSet);
		}
	...
	public void resetUsageParameter(String strServiceID, String strSbbID,
			String parameterSet, String parameterName, boolean isCounterType)
			throws ManagementConsoleException {
