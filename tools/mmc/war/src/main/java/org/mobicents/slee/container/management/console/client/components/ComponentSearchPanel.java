...
public class ComponentSearchPanel extends Composite {
	...
	public void onSearch() {
		ServerCallback callback = new ServerCallback(this) {
			...
			public void onSuccess(Object result) {
				...
				if (componentInfos == null || componentInfos.length == 0) {
					...
					ComponentListPanel componentListPanel = new ComponentListPanel(
							browseContainer, componentInfos);
		...
		try {
			ComponentSearchParams params = new ComponentSearchParams(nameBox
					.getText(), idBox.getText(), vendorBox.getText(),
					versionBox.getText());
