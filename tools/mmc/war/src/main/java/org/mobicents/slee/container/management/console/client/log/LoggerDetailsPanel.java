...
public class LoggerDetailsPanel extends Composite implements CommonControl {
	...
	public LoggerDetailsPanel(BrowseContainer myDisplay, String name,
			String fullName, LogTreeNode node) {
		super();
		...
		private void doInitAddPanes(LoggerInfo info) {
			...
			try {
				if (setters == null) {
					...
					setters.add(new AddSocketHandlerPanel(fullName,shower),
							"Add socket handler");
					setters.add(new AddNotificationHadlerPanel(fullName,shower),
							"Add notifiaction handler");
					setters.add(new AddGenericHadlerPanel(fullName,shower),
							"Add generic handler");
