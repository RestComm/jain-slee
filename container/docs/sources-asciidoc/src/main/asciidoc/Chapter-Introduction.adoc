...
{this-platform} is built on top of the open source award winning {jee-platform}, which means that {this-platform} complements with Java Enterprise (JEE) 7 container features, allowing strong convergence of different application models, for even feature richer communication services, for instance, the Web and  can be combined to achieve a more sophisticated and natural user experience. [.line-through]#{this-platform} inherits quality management features and tools from {jee-platform} , such as the JMX Console, {management-platform} Plugins and SNMP Adaptor.#
